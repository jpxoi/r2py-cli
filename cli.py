...
def list_command(
    bucket_name: str = typer.Argument(
        None, help="Bucket name (omit for --buckets)", show_default=False
    ),
    ...
    with_region: bool = typer.Option(
        False, "--with-region", help="Include regions in the bucket list"
    ),
    multipart: bool = typer.Option(
        False, "--multipart", help="List multipart uploads in the bucket"
    ),
    ...
    try:
        if buckets:
            ...
            if prefix:
                typer.echo(
                    "Error: --multipart and --prefix are mutually exclusive.", err=True
                )
                raise typer.Exit(code=1)
            ...
            if not bucket_name:
                typer.echo(
                    "Error: Must provide a bucket name unless using --buckets.",
                    err=True,
                )
                raise typer.Exit(code=1)
...
@app.command()
def create(
    bucket_name: str, region: Region = typer.Option(Region.AUTO, help="AWS region name")
):
...
def delete(
    ...
    object_key: str = typer.Argument(
        None, help="Object key to delete (omit to delete the bucket)"
    ),
    region: Region = typer.Option(Region.AUTO, help="AWS region name"),
):
    ...
    try:
        if object_key:
            ...
            confirm = typer.confirm(
                f"Are you sure you want to delete the bucket '{bucket_name}'?"
            )
            if not confirm:
                typer.echo("Deletion cancelled.")
                return
